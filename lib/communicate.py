...
class Communicate():
  def __init__(self, device, speed, debug=False, quiet=False, timeout=None):
    ...
    if not quiet: print ('Initializing grbl at device: {:}  Please wait 1 second for device...'.format(device))
    kkk="\r\n\r\n"
    s.write(kkk.encode())
...
class FakeSerial():
    ...
    def __getattr__(self, name):
        print ('DEBUG SERIAL: {:}'.format(name))
        return self.p
