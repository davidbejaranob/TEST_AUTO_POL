...
def show_cam_img(cam,et=.5,gan=1,n_imgs=1):
...
# OJO REVISAR EL PUERTO SERIAL EN EL SW DE OPENBUILDS
    with Communicate('COM7', 115200, timeout=0.70,
                     debug=0,
                     quiet=0) as serial:
        x='$$';serial.run(x)
        x='$X';serial.run(x)#Desbloquear
    return(serial)
...
for k in range(int((gno_tot-gno_ini)/gno_paso)+1):#Mover Goniometro
    ...
    for l in range(int(pol_tot/pol_paso)+1):#Rotar Polarizador
        ...
        ft.writeto('{:}_{:03}.fits'.format(pre_arch,kk),imr,header=h,overwrite=True) #>Grabar imagen
        time.sleep(0.05)
        kk=kk+1
        
    z='$J=G91G21Z-'+str((pol_tot-1)/mpol)+'F300';serial.run(z) #para regresar a 0 ojo el -1 es por backlash
    time.sleep(sle_time)
y='$J=G91G21Y-'+str(((k)*gno_paso-2)/mgno)+'F100';serial.run(y) #para regresar a 0 el -2 es por backlash
time.sleep(sle_time*2)
close_cam(cam)
